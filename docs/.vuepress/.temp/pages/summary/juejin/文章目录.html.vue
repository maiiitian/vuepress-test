...
<p>普通 元素用的是 this.$refs.元素ref名.offsetTop</p>
<p>组件 用的是  this.$refs.组件ref名.$el.offsetTop</p>
<p>最后采用在挂载时使用setTimeout，做一个宏任务处理</p>
...
<p>思路：</p>
<p>在文章内容组件获取数据时，通过数据查找h标签，把h1和h2的内容提取出来，传标签size，标签内容，标签id。</p>
<p>我决定用a标签锚点进行转跳，因为掘金官网看着是a标签转跳。</p>
<p>给原本筛选出来的h标签套一层div</p>
<blockquote>
<p>bug发现！！！</p>
<p>同等级同内容的h标签，转跳就会崩坏</p>
<p>直接在h标签上</p>
</blockquote>
...
<p>在 nuxt.config.js 中（这里暂时是nuxt项目，后期变为nuxt3，无需配置</p>
...
<ul>
<li>点击对应标题时，通过点击事件传递当前的标题index，通过定义一个 activeIndex 来判断是哪个导航条高亮样式。</li>
<li>这里通过获取的h标签等级来进行导航条的缩进 :style=&quot;{ marginLeft: `${hItem.id * 10}px` }</li>
</ul>
...
<p>在页面挂载完成后，获取所有需要的h标签距离顶部的距离存入 hTopList 数组。</p>
<blockquote>
<p>bug发现！</p>
<p>如果文章中存在图片，图片还未加载完就获得了hTopList，导致目录点击转跳跟随错位，我明明使用的a标签转跳，img的src加载...。虽然我添加了延时，本地是解决了但是放到服务器上不行，所以本项目中没让数据人员给我在文章中添加图片数据。</p>
<p>我还尝试在文章图片加载完后，再获取hTopList，但也失败了，我觉得这和markdown css里定义的img样式有关。我应该去解决一下。</p>
</blockquote>
...
<p>监听页面滚动，对比页面滚动值scrollTop和标签的高度来判断是否高亮，</p>
...
<p>这里通过滚动目录div来实现。这里的keep参数就是导航条过多时，滚动条高亮处在第七个，其实应该根据导航box的高度对半取整算一下，这里图方便，数了下掘金是在第几个高亮就写了第几个。34 是每条导航 li 标签的高度。</p>
...
<p>在实现文章目录滚动样式，使用到了 <code v-pre>window.addEventListener</code> 监听鼠标滚动。每一次滚动都会调用一次内部函数。这里采用节流，每0.1秒（草）内行数只执行一次。</p>
